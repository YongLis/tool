	...
	public static List<String> getSqlList (String startTime ,int intervalTime) {
		...
		try {
			...
			for (int i = 1; i < 9600; i++) {
				...
				if (newDate.compareTo(sfDateFormat.parse("2020-09-22")) <= 0) {
					// String sql
...
//					String sql = "call report.SP_SETTLE_ALL_PROCESS_DAILY('"+sfDateFormat.format(date)+"')";
					...
					String sql = "call SP_T_ACCT_BALANCE_DAILY(to_date('"+sfDateFormat.format(newDate)+"','yyyy-mm-dd'))";
//					String sql = "call report.SP_TMP_REFUND_FEE_INFO(to_date('"+sfDateFormat.format(newDate)+"','yyyy-mm-dd'),to_date('"+sfDateFormat.format(date)+"','yyyy-mm-dd'))";
	...
	public static void main(String[] args) {
		AwsReportJobTask task = new AwsReportJobTask();
		
		
		try {
//			Thread.currentThread().sleep(1000*3600*6);
			List<String> sqlList = task.getSqlList("2019-06-11", 1);
