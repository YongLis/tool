...
//			boolean bool = createStatement.execute(sql);                
			...
			writeLogFile("D:/log/tmp_order_stage_success.txt", sql);
		} catch (Exception e) {                                                                                  
			e.printStackTrace();    
			System.out.println(sql+"--执行失败");
			writeLogFile("D:/log/tmp_order_stage_fail.txt", sql);
		}                 
	}
...
//			boolean bool = createStatement.execute(sql);                
			...
			cs.execute();                    
			System.out.println(sql+"执行成功");
			writeLogFile("D:/log/TMP_REFUND_ORDER_success.txt", sql);
		} catch (Exception e) {                                                                                  
			e.printStackTrace();    
			System.out.println(sql+"--执行失败");
			writeLogFile("D:/log/TMP_REFUND_ORDER_fail.txt", sql);
		}         
...
//					String sql = "call report.SP_KJSD_FUND_JOBS_DAILY('"+sfDateFormat.format(date)+"')";
					String sql = "call SP_T_ACCT_BALANCE_DAILY('"+sfDateFormat.format(date)+"')";
...
//					String sql = "call report.SP_TMP_REFUND_FEE_INFO(to_date('"+sfDateFormat.format(newDate)+"','yyyy-mm-dd'),to_date('"+sfDateFormat.format(date)+"','yyyy-mm-dd'))";
	...
	public static void main(String[] args) {
		BIReportJobTask2 task = new BIReportJobTask2();
		
		
		try {
//			Thread.currentThread().sleep(1000*3600*6);
			List<String> sqlList = task.getSqlList("2020-07-09", -1);
